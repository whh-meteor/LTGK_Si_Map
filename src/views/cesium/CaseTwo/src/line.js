...
export default {
    mounted() {
        ...
        removeEntityById(id) {
            ...
            let sea = new Cesium.UrlTemplateImageryProvider({
        ...
        initLines(pointList) {
            // 所有点位
            let allDotInfos = pointList.features.map(i => {
                return {
                    coords: i.geometry.coordinates, name: i.properties.type, properties: i.properties,
                }
            })
            // 到信号塔的浮标
            let xhtBuoyDots = allDotInfos.filter(i => i.properties.equiptype === 'buoy' && i.properties.area === 'xht')
            xhtBuoyDots.forEach(i => i.coords = [...i.coords, 100]) // 设置点高度
            // 到卫星的浮标
            let wxBuoyDots = allDotInfos.filter(i => i.properties.equiptype === 'buoy' && i.properties.area === 'wx')
            wxBuoyDots.forEach(i => i.coords = [...i.coords, 100]) // 设置点高度
            // 信号塔
            let xhtDots = allDotInfos.filter(i => i.properties.equiptype === 'xht')
            xhtDots.forEach(i => i.coords = [...i.coords, 9000])
            // 卫星
            let wxDots = allDotInfos.filter(i => i.properties.equiptype === 'wx')
            wxDots.forEach(i => i.coords = [...i.coords, 180000])
            // 基站
            let jzDots = allDotInfos.filter(i => i.properties.equiptype === 'jz')
            jzDots.forEach(i => i.coords = [...i.coords, 100])
            // 中心
            let zxName = ['A0', 'A5']
            let zxDots = allDotInfos.filter(i => i.properties.equiptype === 'marker' && zxName.includes(i.properties.type))
            zxDots.forEach(i => i.coords = [...i.coords, 100])
            // 观测塔
            let gctDots = allDotInfos.filter(i => i.properties.equiptype === 'tower')
            gctDots.forEach(i => i.coords = [...i.coords, 100])
            // 观测塔和海缆连接点
            let gctName = gctDots.map(i => i.properties.area)
            let gctMarkerDots = allDotInfos.filter(i => i.properties.equiptype === 'marker' && gctName.includes(i.properties.type))
            gctMarkerDots.forEach(i => i.coords = [...i.coords, 100])
            // 平台
            let platformDots = allDotInfos.filter(i => i.properties.equiptype === 'platform')
            platformDots.forEach(i => i.coords = [...i.coords, 100])
            // 观测塔和海缆连接点   
            let platformName = platformDots.map(i => i.properties.area)
            let platformMarkerDots = allDotInfos.filter(i => i.properties.equiptype === 'marker' && platformName.includes(i.properties.type))
            ...
            // 所有线数据
            let data = polyline
            let allLineInfo = data.features.map(i => {
            ...
            // 绘制海底管道虚线
            let gdLine = allLineInfo.filter(i => i.properties.name)
            ...
            // 绘制海缆流动线
            let glLine = allLineInfo.filter(i => !i.properties.name)
        ...
        initLineData(dotData1, dotData2, gradient, color, width) {
            let lineData = dotData1.map(i => {
                let dots = dotData2.filter(a => (a.properties.equiptype === i.properties.area || a.properties.type === i.properties.area))
                return dots.map(dot => {
                    return {
            ...
            lineData.forEach((lines, index) => {
                lines.forEach(line => {
                    let entity = window.viewer.entities.getById(line.name);
                    if (!entity) {
                        this.drawLine(line.coords, {
                            width: width || 1,
                            gradient: gradient,
        ...
        drawLine(coords, option, line) {
            ...
            let middle = parseInt(coords.length / 2 || 0)
            let entity = {
                name: line.name,
        ...
        getLineDots(position, num) {
            ...
            let points = position.map(i => {
                return Cesium.Cartesian3.fromDegrees(...i)
            })
            let times = points.map((i, idx) => 0 + idx * 1 / (points.length - 1))
            let spline = new Cesium.HermiteSpline.createNaturalCubic({
                times: times, points: points,
            });
            let positions = [];
            let pointNums = num // 插值个数
            for (let i = 0; i <= pointNums; i++) {
                let cartesian3 = spline.evaluate(i / pointNums);
