...
export default {
  ...
  methods: {
    init() {
      ...
      // this.addGeological("/json/t9_grid.geojson");// 60万地层加密
    ...
    initShipData() {
        ...
        features: [
          {
            ...
            geometry: {
              coordinates: [
                [122.08570642594748, 31.461898501692374],
                [122.93784465873495, 31.406892169864868],
                [123.76824028729777, 31.5188327735149],
              ],
              type: "LineString",
            },
          },
      ...
      for (let i = 0; i < paths.length; i++) {
        ...
        entity.model.scale = new Cesium.CallbackProperty(() => {
          ...
          let result = turf.booleanPointInPolygon(
            turf.point([lon, lat]),
            that.turfWarnArea
          );
          if (result) {
    ...
    drawWall() {
      ...
      let rgba = Cesium.Color.fromCssColorString(
        that.warn ? "#FF0000" : "#00FF33"
      );
    ...
    loadGeoJsonData(url, offset) {
      ...
      fetch(geoJsonUrl)
        ...
        .then((data) => {
          // 获取所有点的高度值
          const heights = data.features.map(
          ...
          const numLevels = 10;
          const levelHeightRange = (maxHeight - minHeight) / numLevels;
          const maxStretchFactor = 20; // 最大拉伸因子

          const points = data.features.map((feature) => {
            const coordinates = feature.geometry.coordinates;
            const originalHeight = coordinates[2];

            // 计算所在的等级
            const level = Math.floor(
              (originalHeight - minHeight) / levelHeightRange
            );
            // 计算拉伸因子：最小等级为1，其他等级线性增加
            const stretchFactor =
              level === 0
                ? 1
                : 1 + ((level - 1) / (numLevels - 2)) * (maxStretchFactor - 1);
            // 计算拉伸因子：最小等级因子为1，其余等级按固定比例增加
            // const stretchFactor = level === 0 ? 1 : Math.pow(1.5, level);
            // 计算拉伸后的高度
            const newHeight = originalHeight * stretchFactor;
            return [coordinates[0], coordinates[1], newHeight - offset];
          });
    ...
    createSurfaceFromPoints(points) {
      ...
      handler.setInputAction((movement) => {
        ...
        if (
          ...
          pickedObject.primitive === primitive
        ) {
          // 当点击了面时，显示简介信息
          this.showInfo();
        }
    ...
    drawFlow(data) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        var points = this.parabolaEquation(
          [item[1] - 0, item[2] - 0, -4000],
          (item[4] - 0) * (Math.PI / 180),
          (item[3] - 0) * 2,
          6
        );
        var pointArr = [];
      ...
      fetch(geoJsonUrl)
        ...
        .then((data) => {
          ...
          function addBatch(startIndex) {
            for (
              let i = startIndex;
              i < Math.min(startIndex + BATCH_SIZE, positions.length);
              i++
            ) {
              pointCollection.add(
                new Cesium.PointPrimitive({
